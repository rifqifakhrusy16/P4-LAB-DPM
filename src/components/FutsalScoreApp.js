...
const FutsalScoreApp = ({ teamAName, teamBName }) => {
  ...
  const handleAddScore = (team) => {
    ...
    if (team === 'A') {
      ...
      if (newScore === 10) {
        ...
        setWinner(`${teamAName.toUpperCase()} MENANG`); // Set the winner
        Alert.alert('Selamat!', `${teamAName} menang!`);
      }
    } else {
      ...
      if (newScore === 10) {
        setGameOver(true); // End the game
        setWinner(`${teamBName.toUpperCase()} MENANG`); // Set the winner
        Alert.alert('Selamat!', `${teamBName} menang!`);
      }
    }
  };
